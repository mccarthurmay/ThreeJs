...
const sunMaterial = new THREE.MeshBasicMaterial({
    color: 0xffffff, // Brightest white (was 0xffffdd slightly yellowish)
    fog: false
});
...
const moonMaterial = new THREE.MeshBasicMaterial({
    color: 0xffffff, // Brightest white (was 0xeeeeff slightly blue)
    fog: false
});
...
export function updateDayNightCycle(deltaTime) {
    ...
    // Moon is on opposite side of planet from sun, but at its own distance
    const moonDirection = new THREE.Vector3().copy(sun.position).normalize().multiplyScalar(-1);
